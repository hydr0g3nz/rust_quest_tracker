...
use crate::{config::config_loader::{get_adventure_secret_env, get_guild_commander_secret_env}, infra::jwt_authentication};
    ...
    if let Some(cookie_header) = req.headers().get(header::COOKIE) {
        if let Ok(cookie_str) = cookie_header.to_str() {
            ...
            if let Some(token) = access_token {
                if let Ok(secret_env) = get_guild_commander_secret_env() {
                    if let Ok(claims) =
                        jwt_authentication::verify_token(secret_env.secret.to_string(), token)
                    {
...
pub async fn adventurers_authorization(
    ...
    if let Some(cookie_header) = req.headers().get(header::COOKIE) {
        if let Ok(cookie_str) = cookie_header.to_str() {
            ...
            if let Some(token) = access_token {
                if let Ok(secret_env) = get_adventure_secret_env() {
                    if let Ok(claims) =
                        jwt_authentication::verify_token(secret_env.secret.to_string(), token)
                    {
